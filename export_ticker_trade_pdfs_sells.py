...
DEFAULT_OUTPUT_DIR = Path(r"C:\DATA\PROJECTS\STOCKS_raw\.pdf exports sells")
...
def filter_grouped_tickers_with_sell(grouped: Dict[str, List[Transaction]]) -> Dict[str, List[Transaction]]:
    return {
        ticker: items
        for ticker, items in grouped.items()
        if any(tx.action == "SELL" for tx in items)
    }
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Export one plain PDF per ticker, only for tickers that have at least one SELL transaction."
    )
    ...
    grouped = filter_grouped_tickers_with_sell(group_by_ticker(all_transactions))
    exported_transaction_count = sum(len(items) for items in grouped.values())
    sell_ticker_count = len(grouped)
    ...
    print(f"Transactions exported for SELL tickers: {exported_transaction_count}")
    print(f"Tickers with at least one SELL: {sell_ticker_count}")
